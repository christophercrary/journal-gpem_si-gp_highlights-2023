    ...
    def __str__(self):
        ...
        for node in self:
            ...
            while len(stack[-1][1]) == stack[-1][0].arity:
                ...
                if prim.arity == 0 and string[0] != 'v':
                    # Constant terminal; change to NumPy array.
                    string = f'np.array([{string}], dtype=np.float32)'
                if len(stack) == 0:
...
def compile(expr, pset):
    ...
    try:
        ...
        return eval(
            code, {'__builtins__': None, 'inf' : float('inf'), 'np' : np} | 
                pset.context, {})
    except MemoryError:
